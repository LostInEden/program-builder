            ...
            <div className="relative" key={section.href}>
              {section.children ? <>
                <button type="button" aria-expanded={expanded} aria-controls={`section-menu-${index}`}
                  className={`${tabClass} ${active || expanded ? "border-b-grass text-ink" : "text-dim hover:bg-slate-50 hover:text-ink"}`}
              ...
              </> : <Link href={section.href} aria-current={active ? "page" : undefined} onClick={() => setOpen(null)} className={`${tabClass} ${active ? "border-b-grass text-ink" : "text-dim hover:bg-slate-50 hover:text-ink"}`}>{label}</Link>}
            </div>
          );
        })}
        <Link href="/settings" aria-current={pathname === "/settings" ? "page" : undefined} onClick={() => setOpen(null)} className={`${tabClass} ${pathname === "/settings" ? "border-b-grass text-ink" : "text-dim hover:bg-slate-50 hover:text-ink"}`}>Settings</Link>
      </div>
